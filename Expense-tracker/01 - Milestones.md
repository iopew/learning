...
Each milestone = one git commit (at least). Rule: SQL lives only in `internal/store/`; every mutation ends in a redirect; auth is milestone 6.

## Milestone 1 checklist (DB days)

- [ ] `git init` own repo in `~/Documents/Developer/expense-tracker/`
